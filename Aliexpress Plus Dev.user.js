...
GM_config.init(
    {
        ...
        'fields': {
            ...
            'getpricefrompage': {
                label: 'Wishlist: Get Pricing and Shipping using unSafeWindow Mode',
                type: 'checkbox',
                default: false
            },
            'UseSideImgs': {
...
var getpricefrompage = GM_config.get('getpricefrompage');
...
function formatPageShipping(text, full = false){
    try{
...
async function findShipping2(sellingpoints, productid, iteminc){
    let shipping = -1;
    if(sellingpoints){
        for (let i = 0; i < sellingpoints.length; i++) {
            if(sellingpoints[i].tagContent && sellingpoints[i].tagContent.tagText){
                document.querySelector('.waitingdiv').innerHTML = "Found embedded shipping data. Processing..";
                if(sellingpoints[i].tagContent.tagText.indexOf("+ Shipping") != -1){
                    shipping = parseFloat(sellingpoints[i].tagContent.tagText.substring(sellingpoints[i].tagContent.tagText.indexOf('$') + 1));
    ...
    if(shipping == -1 && pagesearch){
        document.querySelector('.waitingdiv').innerHTML = "Extracting shipping data from page " + iteminc + "...";
...
async function getPagePricing(url, searchterm){
    return new Promise((response) => {
        GM_xmlhttpRequest ( {
            method:     'GET',
            url:        url,
            onload:     function (responseDetails) {
                let pagedata = formatPageShipping(responseDetails.responseText, true);
                let skuModule = pagedata.skuModule;
                let propprice = -1;
                let proppriceshipping = -1;
                for (let i = 0; i < skuModule.productSKUPropertyList[0].skuPropertyValues.length; i++){
                    let propitem = skuModule.productSKUPropertyList[0].skuPropertyValues[i];
                    let propname = propitem.propertyValueDisplayName;
                    if(searchterm.indexOf(propname) != -1 || propname.indexOf(searchterm) != -1){
                        for (let i2 = 0; i2 < skuModule.skuPriceList.length; i2++){
                            let proppriceitem = skuModule.skuPriceList[i2];
                            if(propitem.propertyValueId == proppriceitem.skuPropIds){
                                propprice = proppriceitem.skuVal.skuActivityAmount.value;
                                proppriceshipping = (proppriceitem.skuVal.skuAmount.value - propprice);
                            }
                        }
                    }
                }
                if(propprice == -1 || proppriceshipping == -1){
                    let priceModule = pagedata.priceModule;
                    propprice = priceModule.maxActivityAmount.value;
                    proppriceshipping = (priceModule.maxAmount.value - propprice);
                }
                response([propprice.toString(), proppriceshipping.toString()]);
            }
        } );
    });
}
...
async function processall3(runparams = null){
    if(!document.querySelector('.waitingdiv')){
        let waitingdiv = document.createElement('div');
        waitingdiv.className = 'waitingdiv';
        document.querySelector('.top-refine').prepend(waitingdiv);
    }
    if(!runparams){
    ...
    for (let i = 0; i < allitems.length; i++) {
        let iteminc = i.toString() + "/" + allitems.length.toString();
        document.querySelector('.waitingdiv').innerHTML = "Adding item " + iteminc + "...";
        let productid = allitems[i].productId, imgsrc = allitems[i].image.imgUrl, title = allitems[i].title.displayTitle, storename = "", storelink = "", price = "", shipping = "";
        if(getpricefrompage){
            document.querySelector('.waitingdiv').innerHTML = "Extracting shipping data from page " + iteminc + "...";
            let pageparams = await getPagePricing('https://www.aliexpress.com/item/' + productid + '.html');
            price = pageparams[0];
            shipping = pageparams[1];
        }
        else{
            price = allitems[i].prices.salePrice.minPrice;
            shipping = await findShipping2(allitems[i].sellingPoints, productid, iteminc);
        }
        if(allitems[i].store){
            storename = allitems[i].store.storeName;
            storelink = allitems[i].store.storeUrl;
        }
        let extraitems = [];
        if(getextraitems){
    ...
    document.querySelector('.waitingdiv').innerHTML = "Sorting items...";
    SortRows(sortmethod);
    document.querySelector('.waitingdiv').innerHTML = "";
}
...
async function docalctotal(itempageprice){
    let pmain = document.querySelector('.product-main');
    let itempageshipping;
    if(!itemsunsafewindowmode){
        itempageshipping = pmain.querySelector('.product-shipping-price') || pmain.querySelector('.dynamic-shipping-titleLayout');
        if(itempageshipping){
    ...
    itempageprice = itempageprice.substring(itempageprice.indexOf('$')+1);
    if(itempageprice.indexOf(' ') != -1){
        itempageprice = itempageprice.substring(0, itempageprice.indexOf(' '));
    }
    parseFloat(itempageprice);
    let itempagequantity = parseInt(pmain.querySelector('span.product-number-picker span.next-input-group-auto-width > input:nth-child(1)').value);
    itempageprice = itempageprice * itempagequantity;
    let itempagetotal = parseFloat(itempageshipping + itempageprice).toFixed(2).toString();
    shippingtotal = itempagetotal;
    ...
    finalcostdiv.appendChild(finalcostpretext);
    let insertitemtotal = pmain.querySelector('.product-action');
    if(insertitemtotal){
        let pretextitem = pmain.querySelector('.product-info').querySelector('.total-pretext');
        if(pretextitem){
...
function updateprice(itempageprice){
    if(itempageprice.innerText && itempageprice.innerText.indexOf('$') != -1){
        setTimeout(function (){
...
async function monitorquantity(){
    let pmain = document.querySelector('.product-main');
    let itempagequantity = pmain.querySelector('span.product-number-picker span.next-input-group-auto-width > input:nth-child(1)');
    if(itempagequantity){
        itempagequantity.addEventListener('input', function(e){
            updateprice(pmain.querySelector('.product-price-value') || pmain.querySelector('.product-price-current') || pmain.querySelector('.uniform-banner-box-price'));
        });
    }
    let itempagequantitybutton1 = pmain.querySelector('span.product-number-picker span.next-before button:nth-child(1)');
    if(itempagequantitybutton1){
        itempagequantitybutton1.addEventListener('click', function(e){
            updateprice(pmain.querySelector('.product-price-value') || pmain.querySelector('.product-price-current') || pmain.querySelector('.uniform-banner-box-price'));
        });
    }
    let itempagequantitybutton2 = pmain.querySelector('span.product-number-picker span.next-after button:nth-child(1)');
    if(itempagequantitybutton2){
        itempagequantitybutton2.addEventListener('click', function(e){
            updateprice(pmain.querySelector('.product-price-value') || pmain.querySelector('.product-price-current') || pmain.querySelector('.uniform-banner-box-price'));
        });
    }
}
...
async function calctotal(){
    let pmain = document.querySelector('.product-main');
    let itempageprice = pmain.querySelector('.product-price-value') || pmain.querySelector('.product-price-current') || pmain.querySelector('.uniform-banner-box-price');
    if(itempageprice.innerText && itempageprice.innerText.indexOf('$') != -1){
        docalctotal(itempageprice.innerText);
    }
    ...
    let observer4 = new MutationObserver(function(mutationsList, observer) {
        for(const mutation of mutationsList) {
            if(mutation.target.textContent.indexOf(' / lot') != 0){
                docalctotal(mutation.target.textContent);
            }
        }
    });
    observer4.observe(itempageprice, config);
    let proplist = pmain.querySelector('.sku-wrap');
...
function getshippingdates(){
    let pmain = document.querySelector('.product-main');
    let deliverydiv = pmain.querySelector('.product-shipping-delivery');
    if(deliverydiv){
...
function setabs(clicked){
    document.querySelector('.waitingdiv').innerHTML = "Changing page number...";
    let cpage;
    let cpageel;
    ...
    document.querySelector('.waitingdiv').innerHTML = "Loading...";
...
if(document.location.href.indexOf('/wholesale') != -1 || document.location.href.indexOf('/category') != -1 || document.location.href.indexOf('/af') != -1){
    ...
    setTimeout((function(){
        if(!document.getElementById('wishlist-tbody')){
            finalwishliststart(shippingtotal);
        }
    }),3000);
}